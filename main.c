...
static const char version[] =
	"tfwm-" VERSION ", (C)opyright Luca L.\n";
static int (*x_error_handler) (Display *, XErrorEvent *);

static void
usage() {	error("usage: tfwm [-v]\n"); }
...
error_handler(Display *dpy, XErrorEvent *error)
{
	if(error->error_code == BadWindow
		...
		return 0;
	fprintf(stderr, "tfwm: fatal error: request code=%d, error code=%d\n",
...
main(int argc, char *argv[])
{
	...
	dpy = XOpenDisplay(0);
	if(!dpy)
		error("tfwm: cannot connect X server\n");
	...
	if(other_wm_running)
		error("tfwm: another window manager is already running\n");
